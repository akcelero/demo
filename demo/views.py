...
@csrf_exempt
def movies(request):
    return (movies_post if request.method == 'POST' else movies_get)(request)

@csrf_exempt
def comments(request):
    return (comments_post if request.method == 'POST' else comments_get)(request)
...
def serialize_movie(movie):
    serialized = model_to_dict(movie, fields=[field.name for field in movie._meta.fields if field.name != 'id'])
...
def movies_post(request):
    ...
    data.update({k: dateparser.parse(data[k]) for k in ['Released', 'DVD'] if data[k]})
    data['imdbVotes'] = data['imdbVotes'].replace(',','')
    ratings_data = data.pop('Ratings')
    movie = Movie(**data)
    movie.save()

    ratings = [Rating(Source=r['Source'], Value=r['Value'], Movie=movie) for r in ratings_data]
...
def movies_get(request):
    ...
    if 'imdbRatingFrom' in request.GET:
        filters.update({'imdbRating__gte': request.GET.get('imdbRatingFrom', 0.0)})
    if 'imdbRatingTo' in request.GET:
        filters.update({'imdbRating__lte': request.GET.get('imdbRatingTo', 10.0)})

    result = Movie.objects.filter(**filters)
    movies = [serialize_movie(m) for m in list(result)]
    ratings = list(Rating.objects.all())
    [m.update({'Ratings': {'source': r.Source, 'value': r.Value}}) for m in movies for r in ratings if r.Movie == m]
...
def comments_get(request):
    ...
    serialize_comment = lambda c: {'MovieID': c.Movie.pk, 'Text': c.Text} 
    serializable_comments = [serialize_comment(c) for c in comments] if comments else []
...
def comments_post(request):
    ...
    serialize_comment = lambda c: {'MovieID': c.Movie.pk, 'Text': c.Text} 
    serializable_comment = serialize_comment(comment)
